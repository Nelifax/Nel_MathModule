...
class Lattice2():
    ...
    def __init__(self, vectors=list|Matrix, modulo=0, init_time=0):
        ...
        if isinstance(vectors, NelMath.objects.linear_algebra.Matrix):
            ...
            for i in range(len(vectors.columns)):
                self.vectors.append(NelMath.objects.linear_algebra.Vector(vectors.columns[i]))
    ...
    def LLL(self, delta=0.75):
        ...
        def update_mu(mu, kk, latt, latt_ort, mode='k'):
            for i in range(kk):
                if mode=='k':
                    for j in range(i):
                        print(f'Программа не умерла, она усиленно считает уже {int(time.time()-self.init_time)}...[mu_upd](i={i},j={j})                        ',end='\r')
                        mu[i][j]=(latt.vectors[i]*latt_ort.vectors[j])/(latt_ort.vectors[j]*latt_ort.vectors[j])
                else:
                    for j in range(i+1):
                        print(f'Программа не умерла, она усиленно считает уже {int(time.time()-self.init_time)}...[mu_upd](i={i},j={j})                      ',end='\r')
