    ...
    def execute(operand_a: any, operand_b: any):        
        ...
        if isinstance(operand_a, Rational):
            if isinstance(operand_b, Rational):
                from NelMath.objects.math_base.Operators.Minus.OperatorMinusRR import OperatorMinusRR
                return OperatorMinusRR.execute(operand_a, operand_b)
            if isinstance(operand_b, Fraction):
                from NelMath.objects.math_base.Operators.Minus.OperatorMinusFR import OperatorMinusFR
                return -OperatorMinusFR.execute(operand_b, operand_a)
        if isinstance(operand_a, Fraction):
            if isinstance(operand_b, Rational):
                from NelMath.objects.math_base.Operators.Minus.OperatorMinusFR import OperatorMinusFR
                return OperatorMinusFR.execute(operand_a, operand_b)
            if isinstance(operand_b, Fraction):
